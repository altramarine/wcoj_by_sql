...
CREATE TEMP TABLE node0_best AS WITH
  ...
  best_R2 as (SELECT R3.x as x, R3.z as z, 0 as tag, cnt_R2.cnt as cnt FROM R3, cnt_R2 WHERE cnt_R2.z = R3.z),
  cnt_R1 as (SELECT x, COUNT(*) as cnt FROM R1 GROUP BY x),
  best_R1 as (SELECT best_R2.x as x, best_R2.z as z, CASE WHEN best_R2.cnt < cnt_R1.cnt THEN best_R2.tag ELSE 1 END as tag,CASE WHEN best_R2.cnt < cnt_R1.cnt THEN best_R2.cnt ELSE cnt_R1.cnt END as cnt FROM best_R2, cnt_R1 WHERE cnt_R1.x = best_R2.x)
SELECT x, z, tag FROM best_R1;
SELECT COUNT(*) FROM (
