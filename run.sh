...
for qfile in 1.sql 2.sql 3.sql 3cycle.sql 4cycle.sql 5cycle.sql; do
...
numactl --cpunodebind=0 --membind=0 uv run python run_sql.py queries/wcoj/${qfile} -d ${dataset} > ${LOG_DIR}/${dataset}/wcoj_${qfile}.txt
grep "Execution time:" ${LOG_DIR}/${dataset}/wcoj_${qfile}.txt | sed 's/Execution time:/wcoj Execution time:/' >> ${result_file}
