...
usage() {
  echo "Usage: $0 [-d DATASETS] [-t SECONDS] [-o OUTPUT] [-q QUERY_TYPES] [-r] [-b]"
  echo "  DATASETS: names separated by spaces or commas; -d may also be repeated"
  echo "  QUERY_TYPES: query directories separated by spaces or commas, e.g. '3 4' or '3,4'"
  echo "  -b: append queries/normal baselines for the selected datasets to baseline.tsv"
}
...
while getopts ":d:t:o:q:hrb" option; do
  case "$option" in
    d)
      dataset_argument=${OPTARG//,/ }
      for selected_dataset in $dataset_argument; do
...
for dataset in "${datasets[@]}"; do
  case "$dataset" in
    skitters|topcats|gplus|uspatent|epinions|livejournal|twitter|berkstan|google|amazon) ;;
...
if [[ "$baseline" == true ]]; then
  if [[ -z "$output" ]]; then
    output="robustness-test/baseline.tsv"
  fi
  ...
  for dataset in "${datasets[@]}"; do
    log_file="${output%.tsv}-${dataset}.log"
    for query_type in $query_types; do
      ...
      timeout --signal=TERM --kill-after=10s "${timeout_seconds}s" \
        uv run python run_sql.py "$sql_file" -d "$dataset" \
      ...
      printf '%s\t%s\t%s\t%s\n' \
        "$dataset" "${query_type}.sql" "$status" "$execution_time" >> "$output"
      printf '%s/%s: %s (%ss)\n' \
        "$dataset" "${query_type}.sql" "$status" "$execution_time"
    done
    rm -f "$log_file"
  done

  echo "Baseline results appended to $output"
  exit 0
fi

if (( ${#datasets[@]} > 1 )); then
  if [[ -n "$output" ]]; then
    echo "-o can only be used with one dataset outside baseline mode" >&2
    exit 2
  fi

  for dataset in "${datasets[@]}"; do
    command=("$0" -d "$dataset" -t "$timeout_seconds" -q "$query_types")
    if [[ "$resume" == true ]]; then
      command+=(-r)
    fi
    "${command[@]}"
